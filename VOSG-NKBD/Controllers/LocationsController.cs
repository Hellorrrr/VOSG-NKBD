...
namespace VOSG_NKBD.Controllers
{
    public class LocationsController : Controller  
    {
        ...
        public async Task<IActionResult> Index(string searchString, int? pageNumber, string currentFilter, string sortOrder)
        {
            ...
            if (!String.IsNullOrEmpty(searchString))
            {
                locations = locations.Where(l => l.LocationName.Contains(searchString) ||
                    l.Addresss.Contains(searchString) || l.City.Contains(searchString));
            }
        ...
        public async Task<IActionResult> Create([Bind("LocationsID,LocationName,Addresss,Suburb,City,Country,PostalCode,PhoneNumber")] Location location)
        {
        ...
        [HttpPost, ValidateAntiForgeryToken, Authorize(Roles = "Admin")]
        public async Task<IActionResult> Edit(int id, [Bind("LocationsID,LocationName,Addresss,Suburb,City,Country,PostalCode,PhoneNumber")] Location location)
        {
        ...
        private bool LocationExists(int id) => _context.Locations.Any(e => e.LocationsID == id);
